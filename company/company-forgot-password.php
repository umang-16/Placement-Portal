...
if(isset($_POST['reset_request'])){
    ...
    // Check if company email exists in database
    $check = mysqli_query($conn, "SELECT * FROM companies WHERE email='$email' LIMIT 1");
    ...
    if(mysqli_num_rows($check) > 0){
        ...
        mysqli_query($conn, "UPDATE companies SET reset_token='$token', token_expire='$expire' WHERE email='$email'");
        ...
        try {
            ...
            $reset_link = "http://localhost/place_portal/company/company-reset-password.php?token=" . $token;

            $mail->isHTML(true);
            $mail->Subject = "Company Password Reset Request";
            $mail->Body    = "
                <div style='font-family: Arial, sans-serif; background:#f4f6fb; padding:20px;'>
                    <div style='background:#0f172a; padding:20px; border-radius:8px; border-top: 4px solid #38bdf8;'>
                        ...
                        <p style='color:#cbd5e1;'>We received a request to reset the password for your company account.</p>
        ...
        $msg = "<i class='fa fa-triangle-exclamation'></i> Error: This email is not registered with any company.";
        $color = "red";
    }
}
?>
...
<div class="page-container">
    <div class="box">
      <h2><i class="fa fa-building-user" style="color: #38bdf8;"></i> Company Password Reset</h2>
      <p>Enter your official HR email to receive a secure reset link.</p>
